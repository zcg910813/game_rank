function getYuyueData (){
    ...
    const url ='https://www.taptap.com/top/reserve';
    return (async() => {
        ...
        for(let i=0;i<result.urlList.length;i++){
            ...
            const result2 =  await page2.evaluate(() =>{
                let appointmentsNum =document.querySelectorAll('.count-stats')[0]? document.querySelectorAll('.count-stats')[0].innerHTML.replace('人预约','') :'0'
                ...
                return {appointmentsNum,followerNum,score,evaluate,community}
            })
            result.links[i].appointmentsNum = result2.appointmentsNum
